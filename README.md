...
上传商业发票 / 货代报价单 / 空运托书（PDF / 扫描件 / 图片），多模态大模型抽取结构化字段，**代码确定性校验**给出置信度，人工只复核低置信字段，勾稽对账通过后导出 TMS 标准 Excel。
...
- **确定性置信度**：格式正则 / 业务字典（币种·贸易术语·机场三字码·税率档位）/ 数值解析 + **原文回溯定位**——不依赖模型自评
- **三档分流**：≥0.9 预填锁定 ｜ 0.7–0.9 黄色可抽查 ｜ <0.7 标红强制复核（人工只碰该看的）
- **勾稽校验导出闸门**：发票 小计+税额=价税合计、税率×小计≈税额；报价单 费用明细合计=总费用；托书 ETD<ETA、计费重=max(毛重,体积×167)——**不过 = 拦截导出**
